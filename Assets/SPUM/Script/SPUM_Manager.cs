...
public class SPUM_Manager : MonoBehaviour
{
    ...
    public void EnemyToggle()
    {
        _enemyCheck = !_enemyCheck;
    }
    ...
    public void SavePrefabs()
    {
        if(_prefabUnitList.Count < _maxNumber)
        {
            ...
            GameObject child = tSObj.transform.GetChild(0).gameObject;
            AddPlayableComponents(child);
            Destroy(child.GetComponentInParent<UnityEngine.Rendering.SortingGroup>());
            PrefabUtility.SaveAsPrefabAsset(child, "Assets/Resources/Units/" + prefabName + ".prefab");
            DestroyImmediate(tSObj);
    ...
    //Joe function
    private void AddPlayableComponents(GameObject child) {
        child.AddComponent<Rigidbody2D>();
        Destroy(child.GetComponent<UnityEngine.Rendering.SortingGroup>());

        if (_enemyCheck) child.AddComponent<EnemyUnit>();
        else child.AddComponent<UnitController>();

        child.GetComponent<UnitController>().enabled = false;

        foreach(Transform t in child.transform) {
            EditChildSR(t);
        }

        void EditChildSR(Transform t) {
            if (!t) return;

            var spriteRenderer = t.GetComponent<SpriteRenderer>();
            if (spriteRenderer)
                spriteRenderer.sortingLayerName = "Medium";
